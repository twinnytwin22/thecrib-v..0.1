...
const AllCollectorList = (collectors: any, loading: any) => {
  ...
  const currentCollectors = collectors?.collectors?.ownerAddresses.slice(
    indexStart,
    indexEnd
  );
  ...
  return (
    <div className="w-full max-w-sm md:max-w-7xl bg-white border items-center content-center rounded-lg shadow-md p-8 dark:bg-gray-800 dark:border-gray-700">
          ...
          {currentCollectors.map((collector: any, i: any) => (
            <li className="py-3 sm:py-4" key={i}>
              <div className="flex items-center space-x-4">
                {collector.ownerAddress == excludeDead ? null : (
                  <GetENSAvi ownerAddress={collector.ownerAddress} />
                )}
                <div className="flex-1 min-w-0">
                  <div className="text-md tracking-md font-medium text-gray-900 truncate dark:text-white uppercase">
                    {collector.ownerAddress == excludeDead ? null : (
                      <GetENSName ownerAddress={collector.ownerAddress} />
                    )}
                  </div>
                  <div className="text-sm text-gray-500 truncate dark:text-gray-400">
                    {collector.ownerAddress == excludeDead
                      ? null
                ...
                <div className="inline-flex items-center text-base font-semibold text-gray-900 dark:text-white">
                  {collector.ownerAddress == excludeDead
                    ? null
                    : collector.tokenBalances.length}
                </div>
              </div>
            </li>
          ))}
...
export default AllCollectorList;
