...
const fetchCollectors = async (addresses: { contract: any; }[]) => {
  ...
  fetch(`https://eth-mainnet.g.alchemy.com/nft/v2/${ethApiKey}/getOwnersForCollection?contractAddress=${contract}&withTokenBalances=true`, getRequestOptions)
  .then(response => response.json())
  .then(data => {
        return {
          contract,
          collectors: data?.ownerAddresses
          ...
          allAddresses: data?.ownerAddresses
          .map((collector: any) => ({
            collector
          }))
      };
