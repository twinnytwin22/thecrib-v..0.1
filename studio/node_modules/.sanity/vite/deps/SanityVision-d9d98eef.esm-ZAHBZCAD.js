...
// node_modules/@sanity/vision/lib/_chunks/SanityVision-d9d98eef.esm.js
...
function removeNullChildren(children) {
  return import_react.default.Children.toArray(children).filter(function(c2) {
    return c2;
  });
}
...
var TextLeaf = class extends Text2 {
  ...
  sliceString(from, to = this.length, lineSep = "\n") {
    let result = "";
    for (let pos2 = 0, i = 0; pos2 <= to && i < this.text.length; i++) {
      let line2 = this.text[i], end = pos2 + line2.length;
      if (pos2 > from && i)
        result += lineSep;
      if (from < end && to > pos2)
...
var TextNode = class extends Text2 {
  ...
  decompose(from, to, target, open) {
    for (let i = 0, pos2 = 0; pos2 <= to && i < this.children.length; i++) {
      let child = this.children[i], end = pos2 + child.length;
      if (from <= end && to >= pos2) {
  ...
  replace(from, to, text) {
    if (text.lines < this.lines)
      for (let i = 0, pos2 = 0; i < this.children.length; i++) {
        let child = this.children[i], end = pos2 + child.length;
        if (from >= pos2 && to <= end) {
          let updated = child.replace(from - pos2, to - pos2, text);
  ...
  sliceString(from, to = this.length, lineSep = "\n") {
    let result = "";
    for (let i = 0, pos2 = 0; i < this.children.length && pos2 <= to; i++) {
      let child = this.children[i], end = pos2 + child.length;
      if (pos2 > from && i)
        result += lineSep;
      if (from < end && to > pos2)
...
function appendText(text, target, from = 0, to = 1e9) {
  for (let pos2 = 0, i = 0, first = true; i < text.length && pos2 <= to; i++) {
    let line2 = text[i], end = pos2 + line2.length;
    if (end >= from) {
      if (end > to)
...
var ChangeDesc = class {
  ...
  touchesRange(from, to = from) {
    for (let i = 0, pos2 = 0; i < this.sections.length && pos2 <= to; ) {
      let len = this.sections[i++], ins = this.sections[i++], end = pos2 + len;
...
var ChangeSet = class extends ChangeDesc {
  ...
  invert(doc2) {
    ...
    for (let i = 0, pos2 = 0; i < sections.length; i += 2) {
      ...
      if (ins >= 0) {
        ...
        inserted.push(len ? doc2.slice(pos2, pos2 + len) : Text2.empty);
      }
      pos2 += len;
    }
...
var EditorSelection = class {
  ...
  static create(ranges, mainIndex = 0) {
    ...
    for (let pos2 = 0, i = 0; i < ranges.length; i++) {
      let range = ranges[i];
  ...
  static cursor(pos2, assoc = 0, bidiLevel, goalColumn) {
    return SelectionRange.create(pos2, pos2, (assoc == 0 ? 0 : assoc < 0 ? 4 : 8) | (bidiLevel == null ? 3 : Math.min(2, bidiLevel)) | (goalColumn !== null && goalColumn !== void 0 ? goalColumn : 33554431) << 5);
  }
...
var EditorState = class {
  ...
  wordAt(pos2) {
    let { text, from, length } = this.doc.lineAt(pos2);
...
var ContentView = class {
  ...
  domBoundsAround(from, to, offset = 0) {
    ...
    for (let i = 0, pos2 = offset, prevEnd = offset; i < this.children.length; i++) {
      let child = this.children[i], end = pos2 + child.length;
      if (pos2 < from && end > to)
...
function textCoords(text, pos2, side) {
...
var WidgetView = class extends ContentView {
  ...
  coordsAt(pos2, side) {
    ...
    if (!rects.length)
      return Rect0;
    for (let i = pos2 > 0 ? rects.length - 1 : 0; ; i += pos2 > 0 ? -1 : 1) {
      rect = rects[i];
      if (pos2 > 0 ? i == 0 : i == rects.length - 1 || rect.top < rect.bottom)
        break;
    }
...
var CompositionView = class extends WidgetView {
  domAtPos(pos2) {
    ...
    if (!topView)
      return new DOMPos(text, Math.min(pos2, text.nodeValue.length));
    return scanCompositionTree(pos2, 0, topView, text, (v, p) => v.domAtPos(p), (p) => new DOMPos(text, Math.min(p, text.nodeValue.length)));
  }
  sync() {
  ...
  coordsAt(pos2, side) {
    let { topView, text } = this.widget;
    if (!topView)
      return textCoords(text, pos2, side);
    return scanCompositionTree(pos2, side, topView, text, (v, pos3, side2) => v.coordsAt(pos3, side2), (pos3, side2) => textCoords(text, pos3, side2));
  }
  destroy() {
    var _a2;
    super.destroy();
...
function scanCompositionTree(pos2, side, view, text, enterView, fromText) {
  if (view instanceof MarkView) {
    for (let child = view.dom.firstChild; child; child = child.nextSibling) {
      let desc = ContentView.get(child);
      if (!desc)
        return fromText(pos2, side);
      let hasComp = contains(child, text);
      let len = desc.length + (hasComp ? text.nodeValue.length : 0);
      if (pos2 < len || pos2 == len && desc.getSide() <= 0)
        return hasComp ? scanCompositionTree(pos2, side, desc, text, enterView, fromText) : enterView(desc, pos2, side);
      pos2 -= len;
    }
    ...
    return enterView(view, pos2, side);
  }
}
function posFromDOMInCompositionTree(node, offset, view, text) {
  if (view instanceof MarkView) {
    for (let child of view.children) {
      let pos2 = 0, hasComp = contains(child.dom, text);
      if (contains(child.dom, node))
        return pos2 + (hasComp ? posFromDOMInCompositionTree(node, offset, child, text) : child.localPosFromDOM(node, offset));
      pos2 += hasComp ? text.nodeValue.length : child.length;
    }
  } else if (view.dom == text) {
...
function coordsInChildren(view, pos2, side) {
  ...
  function scan(view2, pos3) {
    for (let i = 0, off = 0; i < view2.children.length && off <= pos3; i++) {
      let child = view2.children[i], end = off + child.length;
      if (end >= pos3) {
        if (child.children.length) {
          scan(child, pos3 - off);
        } else if (!after && (end > pos3 || off == end && child.getSide() > 0)) {
          after = child;
          afterPos = pos3 - off;
        } else if (off < pos3 || off == end && child.getSide() < 0) {
          before = child;
          beforePos = pos3 - off;
        }
      }
...
var DocView = class extends ContentView {
  ...
  domAtPos(pos2) {
    let { i, off } = this.childCursor().findPos(pos2, -1);
  ...
  coordsAt(pos2, side) {
    for (let off = this.length, i = this.children.length - 1; ; i--) {
      ...
      if (pos2 > start || pos2 == start && child.type != BlockType.WidgetBefore && child.type != BlockType.WidgetAfter && (!i || side == 2 || this.children[i - 1].breakAfter || this.children[i - 1].type == BlockType.WidgetBefore && side > -2))
        return child.coordsAt(pos2 - start, side);
      off = start;
    }
  }
  measureVisibleLineHeights(viewport) {
    ...
    for (let pos2 = 0, i = 0; i < this.children.length; i++) {
      let child = this.children[i], end = pos2 + child.length;
      if (end > to)
        break;
      if (pos2 >= from) {
  ...
  childCursor(pos2 = this.length) {
    ...
    if (i)
      pos2 -= this.children[--i].length;
    return new ChildCursor(this.children, pos2, i);
  }
  computeBlockGapDeco() {
    let deco = [], vs = this.view.viewState;
    for (let pos2 = 0, i = 0; ; i++) {
      let next = i == vs.viewports.length ? null : vs.viewports[i];
      let end = next ? next.from - 1 : this.length;
      if (end > pos2) {
        let height = vs.lineBlockAt(end).bottom - vs.lineBlockAt(pos2).top;
...
function compositionSurroundingNode(view) {
  ...
  if (cView instanceof LineView) {
    ...
    let pos2 = prev ? ContentView.get(prev).posAtEnd : cView.posAtStart;
    return { from: pos2, to: pos2, node: topNode, text: textNode };
  } else {
    for (; ; ) {
      let { parent } = cView;
      if (!parent)
...
function posAtCoords(view, { x, y }, precise, bias = -1) {
  ...
  if (element && ((_a2 = view.docView.nearest(element)) === null || _a2 === void 0 ? void 0 : _a2.isEditable) != false) {
    if (doc2.caretPositionFromPoint) {
      let pos2 = doc2.caretPositionFromPoint(x, y);
      if (pos2)
...
function moveVertically(view, start, forward, distance) {
  ...
  for (let extra = 0; ; extra += 10) {
    ...
    if (curY < rect.top || curY > rect.bottom || (dir < 0 ? pos2 < startPos : pos2 > startPos))
      return EditorSelection.cursor(pos2, start.assoc, void 0, goal);
  }
}
function skipAtoms(view, oldPos, pos2) {
  ...
  for (; ; ) {
    ...
    for (let set of atoms) {
      set.between(pos2.from - 1, pos2.from + 1, (from, to, value) => {
        if (pos2.from > from && pos2.from < to) {
          pos2 = oldPos.head > pos2.from ? EditorSelection.cursor(from, 1) : EditorSelection.cursor(to, -1);
          moved = true;
        }
      });
    }
...
function findPositionSide(view, pos2, x, y) {
...
var HeightMapGap = class extends HeightMap {
  ...
  forEachLine(from, to, doc2, top2, offset, f) {
    let { firstLine, lineHeight } = this.lines(doc2, offset);
    for (let pos2 = Math.max(from, offset), end = Math.min(offset + this.length, to); pos2 <= end; ) {
...
var ViewState = class {
  ...
  updateForViewport() {
    ...
    for (let i = 0; i <= 1; i++) {
      let pos2 = i ? main.head : main.anchor;
  ...
  lineBlockAt(pos2) {
    return pos2 >= this.viewport.from && pos2 <= this.viewport.to && this.viewportLines.find((b) => b.from <= pos2 && b.to >= pos2) || scaleBlock(this.heightMap.lineAt(pos2, QueryType.ByPos, this.state.doc, 0, 0), this.scaler);
  }
...
function attrsFromFacet(view, facet, base2) {
  for (let sources = view.state.facet(facet), i = sources.length - 1; i >= 0; i--) {
    let source2 = sources[i], value = typeof source2 == "function" ? source2(view) : source2;
    if (value)
...
var RectangleMarker = class {
  ...
  static forRange(view, className, range) {
    if (range.empty) {
      ...
      return [new RectangleMarker(className, pos2.left - base2.left, pos2.top - base2.top, null, pos2.bottom - pos2.top)];
    } else {
  ...
  function drawForLine(from2, to2, line2) {
    ...
    for (let r of view.visibleRanges)
      if (r.to > start && r.from < end) {
        for (let pos2 = Math.max(r.from, start), endPos = Math.min(r.to, end); ; ) {
...
var dropCursorPos = StateField.define({
  ...
  update(pos2, tr) {
    ...
    return tr.effects.reduce((pos3, e) => e.is(setDropCursorPos) ? e.value : pos3, pos2);
  }
});
var drawDropCursor = ViewPlugin.fromClass(class {
  ...
  readPos() {
    let pos2 = this.view.state.field(dropCursorPos);
    let rect = pos2 != null && this.view.coordsAtPos(pos2);
    if (!rect)
      return null;
  ...
  drawCursor(pos2) {
    if (this.cursor) {
      if (pos2) {
        this.cursor.style.left = pos2.left + "px";
        this.cursor.style.top = pos2.top + "px";
        this.cursor.style.height = pos2.height + "px";
      } else {
...
function iterMatches(doc2, re, from, to, f) {
  re.lastIndex = 0;
  for (let cursor2 = doc2.iterRange(from, to), pos2 = from, m; !cursor2.next().done; pos2 += cursor2.value.length) {
...
var activeLineHighlighter = ViewPlugin.fromClass(class {
  ...
  getDeco(view) {
    ...
    for (let r of view.state.selection.ranges) {
      let line2 = view.lineBlockAt(r.head);
...
var tooltipPlugin = ViewPlugin.fromClass(class {
  ...
  writeMeasure(measured) {
    ...
    for (let i = 0; i < this.manager.tooltips.length; i++) {
      ...
      let pos2 = measured.pos[i], size = measured.size[i];
      if (!pos2 || pos2.bottom <= Math.max(editor.top, space2.top) || pos2.top >= Math.min(editor.bottom, space2.bottom) || pos2.right < Math.max(editor.left, space2.left) - 0.1 || pos2.left > Math.min(editor.right, space2.right) + 0.1) {
      ...
      let left = size.width > space2.right - space2.left ? ltr ? space2.left : space2.right - size.width : ltr ? Math.min(pos2.left - (arrow ? 14 : 0) + offset.x, space2.right - width) : Math.max(space2.left, pos2.left - width + (arrow ? 14 : 0) - offset.x);
      let above = !!tooltip.above;
      if (!tooltip.strictSide && (above ? pos2.top - (size.bottom - size.top) - offset.y < space2.top : pos2.bottom + (size.bottom - size.top) + offset.y > space2.bottom) && above == space2.bottom - pos2.bottom > pos2.top - space2.top)
        above = !above;
      let spaceVert = (above ? pos2.top - space2.top : space2.bottom - pos2.bottom) - arrowHeight;
      ...
      if (arrow)
        arrow.style.left = `${pos2.left + (ltr ? offset.x : -offset.x) - (left + 14 - 7)}px`;
      if (tView.overlap !== true)
...
var HoverPlugin = class {
  ...
  startHover() {
    clearTimeout(this.restartTimeout);
    let { lastMove } = this;
    let pos2 = this.view.contentDOM.contains(lastMove.target) ? this.view.posAtCoords(lastMove) : null;
    if (pos2 == null)
      return;
    let posCoords = this.view.coordsAtPos(pos2);
    if (posCoords == null || lastMove.y < posCoords.top || lastMove.y > posCoords.bottom || lastMove.x < posCoords.left - this.view.defaultCharacterWidth || lastMove.x > posCoords.right + this.view.defaultCharacterWidth)
      return;
    let bidi = this.view.bidiSpans(this.view.state.doc.lineAt(pos2)).find((s) => s.from <= pos2 && s.to >= pos2);
    let rtl = bidi && bidi.dir == Direction.RTL ? -1 : 1;
    let open = this.source(this.view, pos2, lastMove.x < posCoords.left ? -rtl : rtl);
  ...
  mousemove(event) {
    ...
    if (tooltip && !isInTooltip(this.lastMove.target) || this.pending) {
      let { pos: pos2 } = tooltip || this.pending, end = (_a2 = tooltip === null || tooltip === void 0 ? void 0 : tooltip.end) !== null && _a2 !== void 0 ? _a2 : pos2;
      if (pos2 == end ? this.view.posAtCoords(this.lastMove) != pos2 : !isOverRange(this.view, pos2, end, event.clientX, event.clientY, 6)) {
...
function hoverTooltip(source2, options = {}) {
  ...
  return [
    hoverState,
    ViewPlugin.define((view) => new HoverPlugin(view, source2, hoverState, setHover, options.hoverTime || 300)),
...
var Tree = class {
  ...
  iterate(spec) {
    ...
    for (let c2 = this.cursor((spec.mode || 0) | IterMode.IncludeAnonymous); ; ) {
      let entered = false;
      if (c2.from <= to && c2.to >= from && (c2.type.isAnonymous || enter(c2) !== false)) {
        if (c2.firstChild())
          continue;
        entered = true;
      }
...
var TreeBuffer = class {
  ...
  findChild(startIndex, endIndex, dir, pos2, side) {
    ...
    for (let i = startIndex; i != endIndex; i = buffer2[i + 3]) {
      if (checkSide(side, pos2, buffer2[i + 1], buffer2[i + 2])) {
        pick = i;
        if (dir > 0)
          break;
      }
...
function enterUnfinishedNodesBefore(node, pos2) {
  let scan = node.childBefore(pos2);
  while (scan) {
...
var TreeNode = class {
  ...
  nextChild(i, dir, pos2, side, mode = 0) {
    for (let parent = this; ; ) {
  ...
  childAfter(pos2) {
    return this.nextChild(0, 1, pos2, 2);
  }
  childBefore(pos2) {
    return this.nextChild(this._tree.children.length - 1, -1, pos2, -2);
  }
  ...
  enterUnfinishedNodesBefore(pos2) {
    return enterUnfinishedNodesBefore(this, pos2);
  }
...
var TreeCursor = class {
  ...
  childAfter(pos2) {
    return this.enterChild(1, pos2, 2);
  }
  childBefore(pos2) {
    return this.enterChild(-1, pos2, -2);
  }
...
function buildTree(data) {
  ...
  let cursor2 = Array.isArray(buffer2) ? new FlatBufferCursor(buffer2, buffer2.length) : buffer2;
  ...
  function takeNode(parentStart, minPos, children2, positions2, inRepeat) {
    ...
    if (end - start <= maxBufferLength && (buffer3 = findBufferSize(cursor2.pos - minPos, inRepeat))) {
      let data2 = new Uint16Array(buffer3.size - buffer3.skip);
      let endPos = cursor2.pos - buffer3.size, index = data2.length;
  ...
  highlightRange(cursor2, from, to, inheritedClass, highlighters) {
    ...
    if (mounted && mounted.overlay) {
      ...
      for (let i = 0, pos2 = start; ; i++) {
        ...
        pos2 = next.to + start;
        if (pos2 > from) {
          this.highlightRange(inner.cursor(), Math.max(from, next.from + start), Math.min(to, pos2), inheritedClass, innerHighlighters);
          this.startSpan(pos2, cls);
        }
      }
      if (hasChild2)
        cursor2.parent();
...
var Language = class {
  constructor(data, parser2, extraExtensions = [], name2 = "") {
    ...
    this.extension = [
      language.of(this),
      EditorState.languageData.of((state, pos2, side) => state.facet(languageDataFacetAt(state, pos2, side)))
    ].concat(extraExtensions);
  }
  isActiveAt(state, pos2, side = -1) {
    return languageDataFacetAt(state, pos2, side) == this.data;
  }
  findRegions(state) {
...
function languageDataFacetAt(state, pos2, side) {
  let topLang = state.facet(language);
  if (!topLang)
    return null;
  let facet = topLang.data;
  if (topLang.allowsNesting) {
    for (let node = syntaxTree(state).topNode; node; node = node.enter(pos2, side, IterMode.ExcludeBuffers))
...
function getIndentation(context, pos2) {
  ...
  return tree ? syntaxIndentation(context, tree, pos2) : null;
}
var IndentContext = class {
  ...
  textAfterPos(pos2, bias = 1) {
    if (this.options.simulateDoubleBreak && pos2 == this.options.simulateBreak)
      return "";
    let { text, from } = this.lineAt(pos2, bias);
    return text.slice(pos2 - from, Math.min(text.length, pos2 + 100 - from));
  }
  column(pos2, bias = 1) {
    let { text, from } = this.lineAt(pos2, bias);
    let result = this.countColumn(text, pos2 - from);
...
function syntaxIndentation(cx, ast, pos2) {
  return indentFrom(ast.resolveInner(pos2).enterUnfinishedNodesBefore(pos2), pos2, cx);
}
...
function indentFrom(node, pos2, base2) {
  for (; node; node = node.parent) {
    let strategy = indentStrategy(node);
    if (strategy)
      return strategy(TreeIndentContext.create(base2, pos2, node));
  }
  return null;
}
function topIndent() {
...
var TreeIndentContext = class extends IndentContext {
  constructor(base2, pos2, node) {
    super(base2.state, base2.options);
    this.base = base2;
    this.pos = pos2;
    this.node = node;
  }
  static create(base2, pos2, node) {
    return new TreeIndentContext(base2, pos2, node);
  }
  get textAfter() {
...
function matchBrackets(state, pos2, dir, config2 = {}) {
  ...
  for (let cur2 = node; cur2; cur2 = cur2.parent) {
    ...
    if (matches && cur2.from < cur2.to)
      return matchMarkedBrackets(state, pos2, dir, cur2, matches, brackets);
  }
...
function matchPlainBrackets(state, pos2, dir, tree, tokenType, maxScanDistance, brackets) {
  ...
  let iter = state.doc.iterRange(pos2, dir > 0 ? state.doc.length : 0), depth2 = 0;
  for (let distance = 0; !iter.next().done && distance <= maxScanDistance; ) {
    let text = iter.value;
    if (dir < 0)
      distance += text.length;
    let basePos = pos2 + distance * dir;
    for (let pos3 = dir > 0 ? 0 : text.length - 1, end = dir > 0 ? text.length : -1; pos3 != end; pos3 += dir) {
      let found = brackets.indexOf(text[pos3]);
...
function getConfig(state, pos2 = state.selection.main.head) {
  ...
  for (let { from, to } of ranges) {
    ...
    for (let pos2 = from; pos2 <= to; ) {
      ...
      if (line2.from > prevLine && (from == to || to > line2.from)) {
        prevLine = line2.from;
        let token2 = getConfig(state, pos2).line;
        if (!token2)
          continue;
...
var deleteByChar = (target, forward) => deleteBy(target, (pos2) => {
  let { state } = target, line2 = state.doc.lineAt(pos2), before, targetPos;
...
var deleteByGroup = (target, forward) => deleteBy(target, (start) => {
  let pos2 = start, { state } = target, line2 = state.doc.lineAt(pos2);
  let categorize = state.charCategorizer(pos2);
  for (let cat = null; ; ) {
    if (pos2 == (forward ? line2.to : line2.from)) {
      if (pos2 == start && line2.number != (forward ? state.doc.lines : 1))
        pos2 += forward ? 1 : -1;
      break;
    }
...
var deleteToLineEnd = (view) => deleteBy(view, (pos2) => {
  let lineEnd = view.lineBlockAt(pos2).to;
  return pos2 < lineEnd ? lineEnd : Math.min(view.state.doc.length, pos2 + 1);
});
var deleteToLineStart = (view) => deleteBy(view, (pos2) => {
  let lineStart = view.lineBlockAt(pos2).from;
  return pos2 > lineStart ? lineStart : Math.max(0, pos2 - 1);
});
...
function isBetweenBrackets(state, pos2) {
  ...
  if (before && after && before.to <= pos2 && after.from >= pos2 && (closedBy = before.type.prop(NodeProp.closedBy)) && closedBy.indexOf(after.name) > -1 && state.doc.lineAt(before.to).from == state.doc.lineAt(after.from).from)
  ...
  nextOverlapping() {
    for (; ; ) {
      ...
      for (let i = 0, pos2 = start; ; i++) {
        ...
        let match = this.match(code, pos2);
        if (match) {
          this.value = match;
          return this;
        }
        if (i == norm.length - 1)
          break;
        if (pos2 == start && i < str.length && str.charCodeAt(i) == code)
          pos2++;
      }
    }
  }
    ...
    for (let i = 0; i < this.matches.length; i += 2) {
      ...
      if (this.query.charCodeAt(index) == code) {
        if (index == this.query.length - 1) {
          match = { from: this.matches[i + 1], to: pos2 + 1 };
        } else {
          this.matches[i]++;
          keep = true;
        }
...
function toCharEnd(text, pos2) {
  if (pos2 >= text.length)
    return pos2;
  let line2 = text.lineAt(pos2), next;
...
var CompletionTooltip = class {
  constructor(view, stateField) {
    ...
    this.placeInfo = {
      ...
      write: (pos2) => this.positionInfo(pos2),
      key: this
    };
    this.space = null;
    this.currentClass = "";
  ...
  positionInfo(pos2) {
    if (this.info) {
      if (pos2) {
        this.info.style.top = pos2.top;
        this.info.style.bottom = pos2.bottom;
        this.info.style.maxWidth = pos2.maxWidth;
        this.info.className = "cm-tooltip cm-completionInfo cm-completionInfo-" + pos2.class;
      } else {
...
var ActiveResult = class extends ActiveSource {
  ...
  handleUserEvent(tr, type, conf) {
    ...
    if ((this.explicitPos < 0 ? pos2 <= from : pos2 < this.from) || pos2 > to || type == "delete" && cur(tr.startState) == this.from)
      return new ActiveSource(this.source, type == "input" && conf.activateOnTyping ? 1 : 0);
    let explicitPos = this.explicitPos < 0 ? -1 : tr.changes.mapPos(this.explicitPos), updated;
    if (checkValid(this.result.validFor, tr.state, from, to))
      return new ActiveResult(this.source, explicitPos, this.result, from, to);
    if (this.result.update && (updated = this.result.update(this.result, from, to, new CompletionContext(tr.state, pos2, explicitPos >= 0))))
...
var Snippet = class {
  ...
  instantiate(state, pos2) {
    let text = [], lineStart = [pos2];
    let lineObj = state.doc.lineAt(pos2), baseIndent = /^\s*/.exec(lineObj.text)[0];
    for (let line2 of this.lines) {
      if (text.length) {
        let indent = baseIndent, tabs = /^\t*/.exec(line2)[0].length;
        for (let i = 0; i < tabs; i++)
          indent += state.facet(indentUnit);
        lineStart.push(pos2 + indent.length - tabs);
        line2 = indent + line2.slice(tabs);
      }
      text.push(line2);
      pos2 += line2.length + 1;
    }
    let ranges = this.fieldPositions.map((pos3) => new FieldRange(pos3.field, lineStart[pos3.line] + pos3.from, lineStart[pos3.line] + pos3.to));
    return { text, ranges };
  }
  static parse(template) {
    ...
    for (let line2 of template.split(/\r\n?|\n/)) {
      ...
      for (let esc; esc = /\\([{}])/.exec(line2); ) {
        ...
        for (let pos2 of positions)
          if (pos2.line == lines.length && pos2.from > esc.index) {
            pos2.from--;
            pos2.to--;
          }
      }
...
var snippetPointerHandler = EditorView.domEventHandlers({
  mousedown(event, view) {
    let active = view.state.field(snippetState, false), pos2;
    if (!active || (pos2 = view.posAtCoords({ x: event.clientX, y: event.clientY })) == null)
      return false;
    let match = active.ranges.find((r) => r.from <= pos2 && r.to >= pos2);
  ...
  let dont = null, changes = state.changeByRange((range) => {
    ...
    if (next == token2) {
      if (nodeStart(state, pos2)) {
        return {
          changes: { insert: token2 + token2, from: pos2 },
          effects: closeBracketEffect.of(pos2 + token2.length),
          range: EditorSelection.cursor(pos2 + token2.length)
        };
      } else if (closedBracketAt(state, pos2)) {
        let isTriple = allowTriple && state.sliceDoc(pos2, pos2 + token2.length * 3) == token2 + token2 + token2;
        return {
          range: EditorSelection.cursor(pos2 + token2.length * (isTriple ? 3 : 1)),
          effects: skipBracketEffect.of(pos2)
        };
      }
    } else if (allowTriple && state.sliceDoc(pos2 - 2 * token2.length, pos2) == token2 + token2 && (start = canStartStringAt(state, pos2 - 2 * token2.length, stringPrefixes)) > -1 && nodeStart(state, start)) {
      return {
        changes: { insert: token2 + token2 + token2 + token2, from: pos2 },
        effects: closeBracketEffect.of(pos2 + token2.length),
        range: EditorSelection.cursor(pos2 + token2.length)
      };
    } else if (state.charCategorizer(pos2)(next) != CharCategory.Word) {
      if (canStartStringAt(state, pos2, stringPrefixes) > -1 && !probablyInString(state, pos2, token2, stringPrefixes))
        return {
          changes: { insert: token2 + token2, from: pos2 },
          effects: closeBracketEffect.of(pos2 + token2.length),
          range: EditorSelection.cursor(pos2 + token2.length)
...
var Stack2 = class {
  constructor(p, stack2, state, reducePos, pos2, score2, buffer2, bufferBase, curContext, lookAhead = 0, parent) {
    this.p = p;
    this.stack = stack2;
    this.state = state;
  ...
  static start(p, state, pos2 = 0) {
    let cx = p.parser.context;
    return new Stack2(p, [], state, pos2, pos2, 0, [], 0, cx ? new StackContext(cx, cx.start) : null, 0, null);
  }
  get context() {
  ...
  reduce(action) {
    ...
    let start = this.stack[base2 - 2], size = this.reducePos - start;
    if (size >= 2e3) {
    ...
    if (type < parser2.minRepeatTerm || action & 131072) {
      let pos2 = parser2.stateFlag(this.state, 1) ? this.pos : this.reducePos;
  ...
  split() {
    ...
    while (parent && base2 == parent.bufferBase)
      parent = parent.parent;
    return new Stack2(this.p, this.stack.slice(), this.state, this.reducePos, this.pos, this.score, buffer2, base2, this.curContext, this.lookAhead, parent);
  }
...
var StackBufferCursor = class {
  ...
  static create(stack2, pos2 = stack2.bufferBase + stack2.buffer.length) {
    return new StackBufferCursor(stack2, pos2, pos2 - stack2.bufferBase);
  }
  maybeNext() {
    ...
    if (!(side < 0 ? cursor2.childBefore(pos2) : cursor2.childAfter(pos2)))
      for (; ; ) {
        if ((side < 0 ? cursor2.to < pos2 : cursor2.from > pos2) && !cursor2.type.isError)
          return side < 0 ? Math.max(0, Math.min(cursor2.to - 1, pos2 - 25)) : Math.min(tree.length, Math.max(cursor2.from + 1, pos2 + 25));
...
var TokenCache = class {
  ...
  getMainToken(stack2) {
    ...
    let main = new CachedToken(), { pos: pos2, p } = stack2;
    main.start = pos2;
    main.end = Math.min(pos2 + 1, p.stream.end);
    main.value = pos2 == p.stream.end ? p.parser.eofTerm : 0;
    return main;
  }
...
(function(Rec2) {
  ...
  Rec2[Rec2["MaxLeftAssociativeReductionCount"] = 1e3] = "MaxLeftAssociativeReductionCount";
})(Rec || (Rec = {}));
var Parse = class {
  ...
  advance() {
    ...
    if (this.bigReductionCount > 1e3 && stacks.length == 1) {
      let [s] = stacks;
    ...
    if (this.recovering) {
      ...
      if (newStacks.some((s) => s.reducePos > pos2))
        this.recovering--;
  ...
  advanceStack(stack2, stacks, split) {
    ...
    for (let i = 0; i < actions.length; ) {
      ...
      if (verbose)
        console.log(base2 + this.stackID(localStack) + ` (via ${(action & 65536) == 0 ? "shift" : `reduce of ${parser2.getName(action & 65535)}`} for ${parser2.getName(term)} @ ${start}${localStack == stack2 ? "" : ", split"})`);
      if (last)
        return true;
...
var LRParser = class extends Parser {
  ...
  getGoto(state, term, loose = false) {
    ...
    if (term >= table[0])
      return -1;
    for (let pos2 = table[term + 1]; ; ) {
      let groupTag = table[pos2++], last = groupTag & 1;
      let target = table[pos2++];
      if (last && loose)
        return target;
...
var insertSemicolon = new ExternalTokenizer((input, stack2) => {
  let { next } = input;
  if ((next == braceR || next == -1 || stack2.context) && stack2.canShift(insertSemi))
...
var noSemicolon = new ExternalTokenizer((input, stack2) => {
  ...
  if (next == slash && ((after = input.peek(1)) == slash || after == star))
    return;
  if (next != braceR && next != semicolon && next != -1 && !stack2.context && stack2.canShift(noSemi))
    input.acceptToken(noSemi);
}, { contextual: true });
var incdecToken = new ExternalTokenizer((input, stack2) => {
  let { next } = input;
...
function createHighlight(theme2) {
  const c2 = theme2.sanity.color.base;
...
function getLocalStorage(namespace) {
  ...
  function get(key2, defaultVal) {
    ...
    return typeof state[key2] === "undefined" ? defaultVal : state[key2];
  }
...
function isExpanded(keyPath, value) {
  ...
  return isRecord(value) && !segments.some((key2) => isArrayKeyOverLimit(key2));
}
...
function mapToLegacyDetails(details) {
  ...
  const line2 = query.slice(lineStart, query.indexOf("\n", lineStart));
  const column2 = start - lineStart;
...
function dashLine(column2, columnEnd) {
  const line2 = "-".repeat(column2);
  const hats = "^".repeat(columnEnd ? columnEnd - column2 : 1);
  return "".concat(line2).concat(hats);
}
...
//# sourceMappingURL=SanityVision-d9d98eef.esm-ZAHBZCAD.js.map
