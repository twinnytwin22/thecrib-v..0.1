...
var _default = (context, callback) => {
  const opts = context.options;
  const options = context.applyMiddleware('finalizeOptions', opts);
  const timers = {}; // Deep-checking window.location because of react native, where `location` doesn't exist

  const cors = win && win.location && !(0, _sameOrigin.default)(win.location.href, options.url); // Allow middleware to inject a response, for instance in the case of caching or mocking
