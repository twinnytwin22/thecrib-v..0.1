...
import { GetENSName, GetENSAvi } from 'ui/Misc/getENS';
       ...
       <div className="flow-root">
           ...
           {currentCollectors.map((collector: any, i: any) => (
                <li className="py-3 sm:py-4" key={i} >
                    ...
                    {collector.ownerAddress == excludeDead ? null : <GetENSAvi ownerAddress={collector.ownerAddress}/>}                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate dark:text-white uppercase">
                              
                            {collector.ownerAddress == excludeDead ? <img className='rounded-full' width={40} src="/assets/newbgdark.png"/> : <GetENSName ownerAddress={collector.ownerAddress}/>}
                            </p>
