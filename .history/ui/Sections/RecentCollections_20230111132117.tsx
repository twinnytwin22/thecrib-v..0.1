      ...
      <div className="grid gap-8 lg:grid-cols-3">
          ...
          <article className="p-6 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700" key={i}>
              ...
              <div className='flex justify-center mb-3 '>
                  ...
                  <h6 className='text-xs mt-0.5 uppercase text-gray-900 dark:text-white tracking-tight pr-5' > Mint Status:</h6>{collection?.mintactive === true && ActiveIndicator()} {collection?.mintactive === false && NonActiveIndicator()}
                  
                  </div>

              <img className="w-72 h-72 rounded-lg gap-10 object-cover scale-100" src={urlFor(collection?.nftImage).width(200).url()} alt="Collection Image" />

              <div className="flex justify-between items-center mt-5">
                  <div className="flex items-center space-x-4">
                      <h2 className="mb-2 text-2xl uppercase font-bold tracking-tight text-gray-900 dark:text-white"><Link href={`/collections/${collection?.slug.current}`}>{collection.title}</Link></h2>
                      {collection?.holderbenefit === true && isHolderBenefit()}

                  </div>
                  <Link href={`collections/${collection?.slug.current}`} className="inline-flex items-center font-medium text-primary-600 text-gray-900 dark:text-white hover:underline">
                 View Drop                      
