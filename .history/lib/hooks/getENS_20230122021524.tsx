...
export function GetENSAvi(ownerAddress: any) {
    ...
    if (isLoading) return <div>…</div>
     ...
     : <FallbackAvi str={!ensName ? xaddress : ensName}/> }</div>
   }

export const FallbackAvi = (str: any) => {
  const CM = "CM"
 const initials = Object.values(str).toString().slice(1, 3)
     return (
       <>
      <div className="rounded-full bg-black p-2.5 uppercase">{!initials ? CM : initials}</div>
      </>
     )
   }
